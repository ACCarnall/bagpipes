...
class Fit:
	...
	def get_model(self, param):
		...
		if self.Model is None:
			if self.Galaxy.spectrum_exists == True:
				self.Model = Model_Galaxy(self.model_components, self.Galaxy.filtlist, output_specwavs=self.Galaxy.spectrum[:,0])
		...
		if self.Galaxy.no_of_spectra > 1:
			...
			for i in range(self.Galaxy.no_of_spectra-1):
				...
				if self.extra_models[i] is None:
					self.extra_models[i] = Model_Galaxy(self.model_components, output_specwavs=self.Galaxy.extra_spectra[i][:,0])
