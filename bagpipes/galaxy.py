...
class Galaxy:
    ...
    def plot(self):
        ...
        if os.path.exists(working_dir + "/pipes/object_masks/" + self.ID + "_mask") and self.spectrum_exists:
            mask = np.loadtxt(working_dir + "/pipes/object_masks/" + self.ID + "_mask")
    ...
    def get_eff_wavs(self):
        filtlist = np.loadtxt(working_dir + "/pipes/filters/" + self.filtlist + ".filtlist", dtype="str")
        for i in range(len(self.photometry)):
            filt = np.loadtxt(working_dir + "/pipes/filters/" + filtlist[i])
