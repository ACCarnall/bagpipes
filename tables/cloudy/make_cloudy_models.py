...
if not os.path.exists(os.environ["CLOUDY_DATA_PATH"] + "/cloudy_lines.txt"):
	os.system("cp " + pipes.setup.install_dir + "/tables/cloudy/cloudy_lines.txt " + os.environ["CLOUDY_DATA_PATH"] + "/pipes_cloudy_lines.txt")
...
def make_cloudy_sed_file(age, zmet):
	...
	np.savetxt(os.environ["CLOUDY_DATA_PATH"] + "/SED/" + pipes.setup.model_type + "_age_" + "%.5f" % age + "_zmet_" + "%.3f" % zmet + ".sed", out_spectrum[::-1, :], header="Energy units: Rydbergs, Flux units: erg/s/Hz")
...
def get_bagpipes_spectrum(age, zmet, out_units_spec="ergscma"):
	...
	model = pipes.Model_Galaxy(model_comp, output_specwavs=pipes.setup.gridwavs[pipes.setup.model_type], out_units_spec=out_units_spec)
...
def make_cloudy_input_file(age, zmet, logU):
	...
	if not os.path.exists("cloudy_temp_files"):
		os.mkdir("cloudy_temp_files")

	if not os.path.exists("cloudy_temp_files/" + pipes.setup.model_type):
		os.mkdir("cloudy_temp_files/" + pipes.setup.model_type)

	path = "cloudy_temp_files/" + pipes.setup.model_type + "/"
...
def run_cloudy_model(age, zmet, logU):
	...
	os.chdir("cloudy_temp_files/" + pipes.setup.model_type + "/" + "logU_" + "%.1f" % logU + "_zmet_" + "%.3f" % zmet)
...
def run_cloudy_grid(nthread, nthreads):
	...
	#for logU in pipes.setup.logU_grid:
	for i in range(nthread, pipes.setup.logU_grid.shape[0], nthreads):
		logU = pipes.setup.logU_grid[i]
		for zmet in pipes.setup.zmet_vals[pipes.setup.model_type]:
			for age in pipes.setup.chosen_ages[pipes.setup.chosen_ages < 2.5*10**7]:
...
def extract_cloudy_results(age, zmet, logU, test=False):
	...
	cloudy_lines = np.loadtxt("cloudy_temp_files/" + pipes.setup.model_type + "/logU_" + "%.1f" % logU + "_zmet_" + "%.3f" % zmet + "/" + "%.5f" % age + ".lines", usecols=(1), delimiter="\t", skiprows=2)
	cloudy_cont = np.loadtxt("cloudy_temp_files/bc03_miles/logU_" + "%.1f" % logU + "_zmet_" + "%.3f" % zmet + "/" + "%.5f" % age + ".econ", usecols=(0,2))[::-1,:]
	...
	cloudy_cont_resampled = np.zeros((pipes.setup.gridwavs[pipes.setup.model_type].shape[0], 2))

	# Resample the nebular continuum onto the wavelength basis of the stellar models
	cloudy_cont_resampled[:,0] = pipes.setup.gridwavs[pipes.setup.model_type]
	cloudy_cont_resampled[:,1] = np.interp(cloudy_cont_resampled[:,0], cloudy_cont[:,0], cloudy_cont[:,1])

	if test == True:
		old_cont = np.loadtxt("../../models/bc03_miles/cloudy/zmet_" + "%.1f" % zmet + "_logU_" + "%.1f" % logU + ".neb_cont")[:2,1:].T
		old_lines = np.loadtxt("../../models/bc03_miles/cloudy/zmet_" + "%.1f" % zmet + "_logU_" + "%.1f" % logU + ".neb_lines")[1,1:]
...
def compile_cloudy_grid():
	...
	for logU in pipes.setup.logU_grid:
		for zmet in pipes.setup.zmet_vals[pipes.setup.model_type]:
			...
			contgrid = np.zeros((pipes.setup.chosen_ages[pipes.setup.chosen_ages < 2.5*10**7].shape[0]+1, pipes.setup.gridwavs[pipes.setup.model_type].shape[0]+1))
			contgrid[0,1:] = pipes.setup.gridwavs[pipes.setup.model_type]
			contgrid[1:,0] = pipes.setup.chosen_ages[pipes.setup.chosen_ages < 2.5*10**7]/10**9

			linegrid = np.zeros((pipes.setup.chosen_ages[pipes.setup.chosen_ages < 2.5*10**7].shape[0]+1, line_wavs.shape[0]+1))
			linegrid[0,1:] = line_wavs
			linegrid[1:,0] = pipes.setup.chosen_ages[pipes.setup.chosen_ages < 2.5*10**7]

			for i in range(pipes.setup.chosen_ages[pipes.setup.chosen_ages < 2.5*10**7].shape[0]):
				age = pipes.setup.chosen_ages[pipes.setup.chosen_ages < 2.5*10**7][i]
			...
			if not os.path.exists("cloudy_temp_files/" + pipes.setup.model_type + "/grids"):
				os.mkdir("cloudy_temp_files/" + pipes.setup.model_type + "/grids")

			np.savetxt("cloudy_temp_files/" + pipes.setup.model_type + "/grids/" + "zmet_" + str(zmet/0.02) + "_logU_" + str(logU) + ".neb_lines", linegrid)
			np.savetxt("cloudy_temp_files/" + pipes.setup.model_type + "/grids/" + "zmet_" + str(zmet/0.02) + "_logU_" + str(logU) + ".neb_cont", contgrid)
